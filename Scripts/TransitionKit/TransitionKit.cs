...
namespace Prime31.TransitionKit
{
    public class TransitionKit : MonoBehaviour
    {
        ...
        {
            CameraRender, CanvasRender
        }

        public TransitionRenderMode renderMode = TransitionRenderMode.CanvasRender;
        ...
        private Canvas transitionKitCanvas;
        ...
        void initialize()
        {
            ...
            if (renderMode == TransitionRenderMode.CameraRender)
            {
            ...
            {
                _instance.StartCoroutine(_instance.setupCanvasAndTexture());
            }
        }
        ...
        {
            var halfHeight = 5f + 0.1f; // 5 is the camera.orthoSize which is the half height
            var halfWidth = halfHeight * ((float)Screen.width / (float)Screen.height) + 0.1f;
        ...
        IEnumerator setupCanvasAndTexture()
        {
            yield return new WaitForEndOfFrame();
            ...
            //Create our canvas to cover the screen
            transitionKitCanvas = getOrAddComponent<Canvas>();
            transitionKitCanvas.renderMode = RenderMode.ScreenSpaceOverlay;
            transitionKitCanvas.sortingOrder = 35;
            transitionKitCanvas.enabled = true;
        ...
        private void cleanup()
        {
            ...
            if (keepTransitionKitInstance)
            {
                ...
                {
                    GetComponent<RawImage>().material.mainTexture = null;
                }
                ...
                {
                    transitionKitCanvas.enabled = false;
                }
            }
            else
            {
        ...
        public IEnumerator waitForLevelToLoad(string sceneName)
        {
        ...
        public IEnumerator tickProgressPropertyInMaterial(float duration, bool reverseDirection = false)
        {
            ...
            {
                elapsed += deltaTime;
                var step = Mathf.Lerp(start, end, Mathf.Pow(elapsed / duration, 2f));//2f is like an interpolation.                
                //float step = Mathf.Max(start, Math.Min(end, elapsed/ Mathf.Max(duration, Mathf.Epsilon)));
